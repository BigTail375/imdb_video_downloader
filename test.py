            ...
            if video_id_list:
                for video_id in video_id_list:
                    try:
                        ...
                        utterances = extract_subtitle('audio.mp3')
                        convert_subtitle(utterances)
                        translate_subtitle('subtitle.srt', list(top_languages.keys()))
                        for key in top_languages.keys():
                            temp_tag_list = tag_list[:]
                            temp_tag_list.append(top_languages[key])
                            combine_subtitle('video.mp4', f'subtitle/subtitle_{key}.srt', 'subtitle.mp4')
                            process_video('subtitle.mp4', temp_tag_list)
                        os.remove('audio.mp3')
                        os.remove('video.mp4')
